"""
Rigorous Statistical Edge Validation
=====================================

This script performs comprehensive statistical tests to determine if the
All-Weather Strategy has a REAL, statistically significant edge.

Tests include:
1. In-Sample vs Out-of-Sample Performance
2. Sharpe Ratio Significance (t-test, bootstrap CI)
3. Monte Carlo Simulation (luck vs skill)
4. Walk-Forward Validation
5. Regime Analysis (Bull/Bear/High-Vol)
6. Information Ratio and Alpha
7. Probabilistic Sharpe Ratio (Bailey & Lopez de Prado)

Author: Statistical Validation Module
"""
...
# =============================================================================
# DATA FETCHING - REAL MARKET DATA
# =============================================================================

def fetch_real_data(years: int = 10) -> Tuple[pd.DataFrame, pd.Series]:
    """
    ...
    try:
        ...
        print(f"Loaded {len(prices)} days of real data for {len(prices.columns)} assets")
        return prices, vix
...
def test_sharpe_significance(returns: pd.Series, benchmark_sharpe: float = 0) -> Dict:
    ...
    Uses the Lo (2002) standard error formula and t-test.
    """
    n = len(returns)
    if n < 30:
...
def probabilistic_sharpe_ratio(returns: pd.Series, 
    ...
    # Standard error including higher moments (Bailey & Lopez de Prado 2012)
...
def monte_carlo_test(returns: pd.Series, n_simulations: int = 10000) -> Dict:
    ...
    Generates random returns with same mean/std and calculates what
    percentage of random strategies beat the actual strategy.
    """
    ...
    for _ in range(n_simulations):
        # Random returns from same distribution
...
def regime_analysis(returns: pd.Series, vix: pd.Series) -> Dict:
    ...
    """
    # Align indices and handle VIX potentially being a DataFrame
    if isinstance(vix, pd.DataFrame):
        vix = vix.iloc[:, 0]  # Take first column
...
def calculate_alpha_beta(strategy_returns: pd.Series, 
    ...
    # Regression: strat = alpha + beta * bench + epsilon
...
def run_complete_validation():
    ...
    """
    print("=" * 70)
    print("   RIGOROUS STATISTICAL EDGE VALIDATION")
    print("   Testing All-Weather Strategy on Real Market Data")
    print("=" * 70)
    
    # Fetch real data
    prices, vix = fetch_real_data(years=10)
    ...
    split_point = int(len(prices) * 0.7)  # 70% in-sample, 30% OOS
    ...
    # Define strategies (from PDF: Compounder Detection Strategy)
    strategies = {
    ...
    spy_returns = prices['S&P 500'].pct_change().iloc[split_point:].dropna()
    ...
    for name, res in results.items():
        ...
        alpha_beta = calculate_alpha_beta(strat_returns, spy_returns)
    ...
    # ==========================================================================
    print("\n" + "=" * 70)
    print("   FINAL VERDICT: DO YOU HAVE A REAL EDGE?")
    print("=" * 70)
    ...
    alpha_beta = calculate_alpha_beta(aw_oos_returns, spy_returns)
    ...
    if evidence_for >= 8 and evidence_against <= 1:
        ...
        print("   The All-Weather Strategy demonstrates statistically")
        print("   significant outperformance that is unlikely due to luck.")
        ...
        print("\n   VERDICT: WEAK EVIDENCE OF EDGE")
        print("   Some positive signals but not conclusive.")
    else:
        print("\n   VERDICT: INSUFFICIENT EVIDENCE OF EDGE")
