...
from cashflow_analysis import (
    get_cash_forecast_summary,
    get_cash_position_summary,
    analyze_payment_terms_impact,
    get_monthly_spend_comparison,
    calculate_po_delay_savings,
)
        ...
        # MARKET OVERVIEW PAGE
            ...
            with col1:
                ...
                if not df_market.empty:
                    ...
                    view_mode = st.radio(
                        "View Mode",
                        ["Market Monitor", "Command Center", "FIFO Tracker", "Buy Calendar", "Cash Flow", "Hedge Optimizer"],
                        horizontal=True,
                    ...
                    if view_mode == "Command Center":
                    ...
                    elif view_mode == "Cash Flow":
                        st.markdown("---")
                        st.markdown("### >> CASH_FLOW_DASHBOARD")
                        st.caption("Working capital optimization: Cash forecast, payment terms analysis, and spend tracking.")
                        
                        # Fetch cash flow data
                        with st.spinner("Analyzing cash position..."):
                            cash_summary = get_cash_position_summary(cursor)
                        ...
                        with kpi1:
                            st.metric(
                                ...
                                f"${cash_summary.get('next_30_day_outflow', 0):,.0f}",
                                help="Total cash going out from open POs due in next 30 days"
                            )
                        with kpi2:
                            st.metric(
                                "📤 Days 31-60 Outflow",
                                f"${cash_summary.get('next_60_day_outflow', 0):,.0f}",
                                help="Cash outflow for POs due in 31-60 days"
                            )
                        with kpi3:
                            st.metric(
                                "📋 Open PO Count",
                                f"{cash_summary.get('open_po_count', 0):,}",
                                help="Number of open purchase orders"
                            )
                        with kpi4:
                            st.metric(
                                "💰 7-Day Delay Savings",
                                f"${cash_summary.get('potential_delay_savings_7day', 0):,.2f}",
                                delta="@ 5% cost of capital",
                                help="Working capital savings if all POs delayed by 7 days"
                            )
                        
                        st.markdown("---")
                        
                        # === ACCURACY SCORE (Self-Learning) ===
                        accuracy_score = cash_summary.get('accuracy_score', 0)
                        predictions_made = cash_summary.get('predictions_made', 0)
                        vendors_tracked = cash_summary.get('vendors_with_patterns', 0)
                        
                        acc_col1, acc_col2, acc_col3 = st.columns([2, 1, 1])
                        with acc_col1:
                            if predictions_made > 0:
                                st.progress(accuracy_score / 100, text=f"🎯 Prediction Accuracy: {accuracy_score:.0f}%")
                            else:
                                st.progress(0.0, text="🎯 Learning... (no predictions yet)")
                        with acc_col2:
                            st.caption(f"📊 {predictions_made} predictions made")
                        with acc_col3:
                            st.caption(f"👥 {vendors_tracked} vendor patterns learned")
                        
                        st.markdown("---")
                        
                        # === TABS FOR DETAILED VIEWS ===
                        cf_tab1, cf_tab2, cf_tab3, cf_tab4 = st.tabs(["📊 Cash Forecast", "🤝 Payment Terms", "📈 Spend Tracking", "🧠 Learning Insights"])
                        
                        with cf_tab1:
                            st.markdown("#### Cash Outflow Forecast (Next 90 Days)")
                            
                            forecast_data = get_cash_forecast_summary(cursor, days_ahead=90)
                            by_week = forecast_data.get("by_week", [])
                            
                            if by_week:
                                df_weekly = pd.DataFrame(by_week)
                                df_weekly['WeekStart'] = pd.to_datetime(df_weekly['WeekStart'])
                                df_weekly = df_weekly.sort_values('WeekStart')
                                
                                # Create a bar chart
                                chart_data = df_weekly.set_index('WeekStart')['Amount']
                                st.bar_chart(chart_data)
                                
                                st.caption(f"Total: **${forecast_data.get('total_outflow', 0):,.0f}** across **{forecast_data.get('po_count', 0)}** POs")
                                
                                # Top Vendors by Outflow
                                st.markdown("##### Top Vendors by Cash Outflow")
                                by_vendor = forecast_data.get("by_vendor", [])
                                if by_vendor:
                                    df_vendors = pd.DataFrame(by_vendor)
                                    st.dataframe(
                        ...
                        with cf_tab2:
                            ...
                            st.caption("Compare vendors by payment terms to identify cash flow optimization opportunities.")
                            
                            df_terms = analyze_payment_terms_impact(cursor)
                            
                            if not df_terms.empty:
                                # Summary stats
                                avg_terms = df_terms['DaysToPayment'].mean()
                                best_terms = df_terms.nlargest(5, 'DaysToPayment')
                                worst_terms = df_terms.nsmallest(5, 'DaysToPayment')
                                
                                col_a, col_b = st.columns(2)
                                with col_a:
                                    st.markdown("##### 🟢 Best Terms (Longest to Pay)")
                                    st.dataframe(
                                        best_terms[['VENDNAME', 'PaymentTerms', 'DaysToPayment', 'Last12MonthSpend']].style.format({
                                            "Last12MonthSpend": "${:,.0f}"
                                        }),
                                        hide_index=True
                                    )
                                with col_b:
                                    st.markdown("##### 🔴 Negotiate These (Shortest Terms)")
                                    st.dataframe(
                                        worst_terms[['VENDNAME', 'PaymentTerms', 'DaysToPayment', 'Last12MonthSpend']].style.format({
                                            "Last12MonthSpend": "${:,.0f}"
                                        }),
                                        hide_index=True
                                    )
                                ...
                                st.markdown("---")
                                st.markdown(f"**Average Payment Terms:** {avg_terms:.0f} days | **Baseline:** Net 30")
                                
                                # Full table
                                with st.expander("View All Vendors by Payment Terms"):
                                    display_terms = df_terms[['VENDORID', 'VENDNAME', 'PaymentTerms', 'DaysToPayment', 'Last12MonthSpend', 'AnnualCashFlowImpact']].copy()
                                    display_terms = display_terms.sort_values('Last12MonthSpend', ascending=False)
                                    st.dataframe(
                                        display_terms.style.format({
                                            "Last12MonthSpend": "${:,.0f}",
                        ...
                        with cf_tab3:
                            st.markdown("#### Monthly Procurement Spend")
                            st.caption("Track spending trends with year-over-year comparison.")
                            
                            df_spend = get_monthly_spend_comparison(cursor, months=24)
                            
                            if not df_spend.empty:
                                # Line chart of spend over time
                                chart_spend = df_spend.set_index('Period')[['Spend', 'PriorYearSpend']]
                                st.line_chart(chart_spend)
                                
                                # YoY summary
                                current_year = datetime.date.today().year
                                df_current = df_spend[df_spend['Year'] == current_year]
                                df_prior = df_spend[df_spend['Year'] == current_year - 1]
                                
                                ytd_spend = df_current['Spend'].sum()
                                prior_ytd = df_prior['Spend'].sum() if not df_prior.empty else 0
                                yoy_change = ((ytd_spend - prior_ytd) / prior_ytd * 100) if prior_ytd > 0 else 0
                                
                                col_s1, col_s2, col_s3 = st.columns(3)
                                with col_s1:
                                    st.metric(f"{current_year} YTD Spend", f"${ytd_spend:,.0f}")
                                with col_s2:
                                    st.metric(f"{current_year - 1} YTD Spend", f"${prior_ytd:,.0f}")
                                with col_s3:
                                ...
                                with st.expander("View Monthly Details"):
                                    st.dataframe(
                                        df_spend[['Period', 'Spend', 'PriorYearSpend', 'YoYChange']].style.format({
                                            "Spend": "${:,.0f}",
                                            "PriorYearSpend": "${:,.0f}",
                                            "YoYChange": "{:+.1f}%"
                                        }),
                        ...
                        with cf_tab4:
                            st.markdown("#### 🧠 Self-Learning Cash Flow System")
                            st.caption("The system learns from actual payment patterns to improve forecast accuracy over time.")
                            
                            # Import the accuracy report function
                            from cashflow_analysis import get_accuracy_report
                            ...
                            with stat_col1:
                                st.metric("Accuracy Score", f"{accuracy_report.get('accuracy_score', 0):.0f}%")
                            with stat_col2:
                                st.metric("Total Predictions", accuracy_report.get('total_predictions', 0))
                            with stat_col3:
                                st.metric("Avg Days Off", f"{accuracy_report.get('avg_days_off', 0):.1f}")
                            
                            st.markdown("---")
                            
                            # Vendor patterns
                            most_predictable = accuracy_report.get('most_predictable_vendors', [])
                            least_predictable = accuracy_report.get('least_predictable_vendors', [])
                            
                            if most_predictable:
                                st.markdown("##### ✅ Most Reliable Vendors (Predictable Payment Timing)")
                                for v in most_predictable[:5]:
                                    days_text = f"{v['avg_days_offset']:+.1f} days" if v['avg_days_offset'] != 0 else "On time"
                                    st.markdown(f"- **{v['vendor_id']}**: {days_text} ({v['data_points']} data points)")
                            
                            if least_predictable:
                                st.markdown("##### ⚠️ Variable Vendors (Less Predictable)")
                                for v in least_predictable[:5]:
                                    st.markdown(f"- **{v['vendor_id']}**: {v['avg_days_offset']:+.1f} days ({v['data_points']} data points)")
                            
                            st.markdown("---")
                            st.markdown("##### 📈 How Learning Works")
                            st.info("""
                            **The system gets smarter over time:**
                            1. **Tracks actual payments** - Compares predicted due dates to when invoices were actually paid
                            2. **Learns vendor patterns** - Some vendors consistently pay early/late
                            3. **Applies adjustments** - Future forecasts are adjusted based on learned patterns
                            4. **Improves accuracy** - The more data, the better the predictions
                            
                            The system automatically learns from the last 30 days of paid invoices each time you view this dashboard.
                            """)
                            
                            if accuracy_report.get('last_updated'):
                                st.caption(f"Last learning update: {accuracy_report.get('last_updated')}")
